...
** TODO Get rid of `maxRows` and `maxCols` in `PanelShape` and switch to manual pagination, do it only when user wants: ability to disable / enable pagination from builder + Move paging inside nested controls;
** DONE Detachable: Add `clientId` to the URL when it was generated (so that reloading the page won't lose changes);
** TODO Detachable: Hide the user view by default, when interface is detached;
...
** TODO BREAKING? send some special value with `RawOutUpdate` for `Choice` controls or else it is hard to get what was actually chosen;
...
** TODO _Active_ condition for a button, some effect for when it is pressed;
** TODO Vertical pagination;
** TODO Consider selecting the page which current item when update came from JS;
...
** TODO BUG Use labeled paths to communicate with JS, index-based IDs can change if some controls were added/removed, label paths have higher chance to be unique; The safest way is UIDs though;
** TODO Store a value even in `Nil` `Property`, so that property would always have some value, no `Maybe`s, it is easier to `map` the `Property` and always have some value inside; Rename `Nil` to `Ghost`; on the other hand `Ghost`s do not need paths or produce messages.... maybe... maybe `Tron msg` could be `Property (Maybe msg)`;
** TODO BUG sending value from JS to the choice is not switching it to the corresponding page;
** TODO Store pages inside nesting controls, do not redistribute every time;
** TODO Max cols / Max rows should not be needed (replace by enabling/disabling paging);
** TODO Too many `fold`s in `Property`; Split things into modules;
** TODO Events for `Controls` and Controls' `update`/`view` should be inside the module;
...
** TODO Move keyboard & mouse drag-start/dragging/drag-end logic to controls themselves;
...
** TODO `(Path, LabelPath)` pairs are used quite often as well as `Path.advance` & `labelPath ++ [ label ]`, find something egeneric for that cases;
...
** TODO BREAKING? Change choice and nest to work with `Array`s since we usually need to get item by index? But Array syntax is not very friendly for API
